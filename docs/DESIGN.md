...
**Contacts are an entity-level address book, not a query-time aggregation.** `contacts` ← `contact_addresses` is materialized from the archive and refreshed at ingest (and via `maildb contacts refresh`). An automated human-probability classifier writes explainable signals and a score, but never writes `kind` — final kind is always a manual decision, guarded by `kind_source='manual'` so refresh/classify cannot clobber curation. MCP exposes read tools plus a single-contact curation write (`update_contact`); batch/maintenance jobs stay on the CLI (`contacts refresh|classify`). Phase 2: curation is queue-driven — `needs_review` surfaces unclassified contacts ranked by volume × human-probability; bulk kind application is a CLI operation and always `kind_source='manual'`. The classifier gained corpus-level thread signals (user-replied-thread participation, thread depth) and org-name hints; still one set-based pass, still never writes kind. Queries are contact-aware: correspondence by contact spans all addresses; top_contacts can collapse by entity; unreplied can filter to human counterparties (manual kind or probability threshold). Merges are snapshot-audited and reversible (`contact_merges`); merge is the MCP curation write, candidates/unmerge stay on the CLI; the builder cannot resurrect merged sources because address rows move with the merge.
